...
public class RunnerPart2 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < time; i++) {
			...
			Jigsaw jigsaw = new Jigsaw(startNode, destNode);
			jigsaw.ASearch();
			if (max < jigsaw.getSearchedNodesNum())
				max = jigsaw.getSearchedNodesNum();
			if (min > jigsaw.getSearchedNodesNum())
				min = jigsaw.getSearchedNodesNum();
			sum += jigsaw.getSearchedNodesNum();
		}
		System.out.println("max:" + max);
		System.out.println("min:" + min);
		System.out.println("avg:" + sum/time);
	}
